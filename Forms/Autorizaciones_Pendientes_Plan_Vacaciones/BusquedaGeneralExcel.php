...
$sql = "select pv.idplan_vacaciones, pv.cedula, pv.nombres, pv.apellidos, d.nombre, pv.fecha_1_salida, pv.fecha_1_retorno, pv.fecha_2_salida, pv.fecha_2_retorno
, pv.fecha_3_salida, pv.fecha_3_retorno, pv.fecha_4_salida, pv.fecha_4_retorno, pv.fecha_5_salida, pv.fecha_5_retorno
, pv.fecha_6_salida, pv.fecha_6_retorno, case autorizacion when 1 then 'Autorizado' when 0 then 'Rechazado' end as autorizacion, pv.reemplazo from plan_vacaciones pv
inner join departamento d on (d.iddepartamento=pv.unidadoperativa) where idpersonajefe = (select idpersona from persona where cedula = '".$cedula."')";
