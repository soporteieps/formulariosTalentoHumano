...
if($_SESSION["cod_tipo_usuario"]==3)
		{
			echo "<form name='form1' method='post' action='../../../carpetah.php'>";
		}
elseif($_SESSION["cod_tipo_usuario"]==1 || $_SESSION["cod_tipo_usuario"]==2)
		{
			echo "<form name='form1' method='post' action='../../../ingreso_carpeta_administradorh.php'>";
		}
?>
...
if($_SESSION["cod_tipo_usuario"]!=1 && $_SESSION["cod_tipo_usuario"]!=2)
  ...
  while($oDB->db->FetchRow($rsMain,$row)) {
    ...
    for ($i=0; $i < $colcnt; $i++) {
      ...
      if($i==0)
	  {
	  	$v="<a href='../../../carpetah.php?codigo_carpeta=".$v."&action=update'>".$v."</a>";
	  }
	  $grid->AddCell($v);
    }
  }
