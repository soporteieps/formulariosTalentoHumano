...
Protected Function RenderQueryRows(writer as HTMLTextWriter) As Integer
  ...
  if not IsNothing(oParse) then
    ...
    if distinctCol >= 0 then
      sqlText=oParse.UnparseSelectDistinct(distinctCol)
  ...
  rowcnt=0
  if not limitQuery then
    while (rowcnt < offset) and (not eof)
      if rdr.Read() then
        rowcnt += 1
      else
        eof=true
