...
Rico.onLoad( function() {
  ...
  var grid_options = {
   ...
   columnSpecs:  [{filterUI:'t',width:150},{filterUI:'t',width:300}, {filterUI:'t',width:300},
                   {filterUI:'t',width:300}, {filterUI:'t',width:200}]
  };
...
if($_SESSION["cod_tipo_usuario"]==3)
		{
			echo "<form name='form1' method='post' action='../../../carpeta.php'>";
		}
elseif($_SESSION["cod_tipo_usuario"]==1)
		{
			echo "<form name='form1' method='post' action='../../../ingreso_carpeta_administrador.php'>";
		}
?>
...
$sqltext="SELECT f.codigo_carpeta, a.area, d.tipo_documento, t.des_archivo, p.des_informe, f.des_carpeta
          ...
          INNER JOIN tipo_documento d ON (f.cod_documento = d.cod_documento)
...
if($_SESSION["cod_tipo_usuario"]!=1 && $_SESSION["cod_tipo_usuario"]!=2)
  ...
  while($oDB->db->FetchRow($rsMain,$row)) {
    ...
    for ($i=0; $i < $colcnt; $i++) {
      ...
      if($i==0 && ($_SESSION["cod_tipo_usuario"]==1 || $_SESSION["cod_tipo_usuario"]==3))
	  {
	  	$v="<a href='../../../carpeta.php?codigo_carpeta=".$v."&action=update'>".$v."</a>";
	  }
      if($i==0 && ($_SESSION["cod_tipo_usuario"]==2))
	  {
	     $v="<a href='../../../carpeta_c.php?codigo_carpeta=".$v."&action=update'>".$v."</a>";
      }
	  $grid->AddCell($v);
    }
  }
