...
if($accion=="input")
{
	insertFormulario($iddepartamento, $jefeinmediato, $cedula_subrrogante, $fecha_inicio, $fecha_fin, $comentario_jefe);
}
...
function insertFormulario($id_departamento, $id_jefesubrrogado, $id_jefesubrrogante, $fecha_inicio, $fecha_fin, $observacion)
{

	 $inserta="insert into subrrogacion (id_departamento, id_jefesubrrogado, id_jefesubrrogante, fecha_inicio, fecha_fin, observacion)
	values
	('$id_departamento', '$id_jefesubrrogado', '$id_jefesubrrogante', '$fecha_inicio', '$fecha_fin', '$observacion')";
	//echo $inserta.'<br>';
		
	  if(query($inserta))
	  {
		  if(
		  	(usuario($id_jefesubrrogado, '0')) and 
		  	(perfiles($id_jefesubrrogado,$id_jefesubrrogante,'0')) and 
			(dato_personal($id_jefesubrrogado,$id_jefesubrrogante, $fecha_inicio, $fecha_fin,'0')) and 
			(persona($id_jefesubrrogado, $id_jefesubrrogante,'0')))
		  {
			  mensaje("LA SUBRROGACION A SIDO INICIADA SATISFACTORIAMENTE");
		  }
	  }
}
