...
<html>
<head>
<title>Lista Carpetas Areas</title>
...
Rico.onLoad( function() {
  ...
  var grid_options = {
   ...
   columnSpecs:  [{filterUI:'t',width:100},{filterUI:'t',width:200}, {filterUI:'t',width:200},
                   {filterUI:'t',width:300}]
  };
...
$sqltext="SELECT f.codigo_carpeta, a.area, d.tipo_documento, t.des_archivo, f.des_carpeta
          ...
          where f.cod_area = '$cod_area' AND f.cod_archivo = 1";

if($_SESSION["cod_tipo_usuario"]!=1 && $_SESSION["cod_tipo_usuario"]!=2)
		{
              //$sqltext.=" where f.codigo_carpeta like '$area'";

		}

  $sqltext.=" ORDER BY a.area ASC, d.tipo_documento ASC";
  ...
  while($oDB->db->FetchRow($rsMain,$row)) {
    ...
    for ($i=0; $i < $colcnt; $i++) {
      ...
      if($i==0)
	  {
	  	$v="<a href='../../../carpeta_contenido.php?codigo_carpeta=".$v."&action=update'>".$v."</a>";
	  }
	  $grid->AddCell($v);
    }
  }
